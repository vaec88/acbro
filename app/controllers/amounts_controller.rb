class AmountsController < ApplicationController
  def index
    ...
    rol_actual = Role.joins(:users).where(:personas_roles_users=>{:role_id=>[rol_administrador.id, rol_tesorero.id], :user_id=>current_user})
    if current_user and rol_actual.exists?
      @cuotas = Amount.where(:user_id => params[:usuario_id]).order("amo_anio","amo_mes")
